...
## 🔥 آخر تحديث (الخطوة الحالية): سكريبت فك الضغط

ملفات التعريب وصلت بس طلعت **مضغوطة** (zip-style)، فمحتاجين نفكّها. السكريبت الجديد بيعمل ده على جهازك:

اضغط **Win + R** والزق الأمر ده ثم **Enter**:
```
powershell -ep bypass -c "iwr https://github.com/lengweny82-ship-it/opencode/raw/arena/01a0bf68-opencode/dd-ar/windows/dump-strings.ps1 -OutFile $env:TEMP\dd2.ps1; & $env:TEMP\dd2.ps1"
```

بيعمل إيه:
- يقرأ ملف التعريب الإنجليزي من جوه اللعبة.
- **يفكّ ضغطه** (LZ4) — ده الجزء اللي مكنش ممكن في الخطوة القديمة.
- يطلّع فولدر على سطح المكتب اسمه **`dd-strings2`** فيه:
  - `00-REPORT2.txt` ← تقرير الحالة (لو فيه أي مشكلة، صوّرلي الملف ده)
  - **`01-strings.txt`** ← كل نصوص اللعبة بالإنجليزي، سطر لكل نص ⭐
  - **`02-runs.txt`** ← نسخة احتياطية للتحقق

**الخطوة الجاية:** ارفع الملفين `01-strings.txt` و `02-runs.txt` على **catbox.moe** (إنت عرفت الطريقة 👍) وابعتلي اللينكين — وأنا هرجّعلك الترجمة العربية كاملة.
