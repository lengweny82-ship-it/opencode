# =====================================================================
#  dd-send-file2.ps1  -  Double Dealers arabic helper
#
#  takes the file that dd-fontobj2.ps1 already wrote :
#      Desktop\dd-fontobj\FONT-OBJECTS.txt
#  uploads it to six free hosts, checks every link by downloading it
#  back and comparing the fingerprint, then shows ALL working links.
...
function Fixed-Link([string]$link) {
  $m = [regex]::Match($link, "^https?://tmpfiles\.org/(\d+)/(.+)$")
  if ($m.Success) { return ("https://tmpfiles.org/dl/" + $m.Groups[1].Value + "/" + $m.Groups[2].Value) }
  return $link
}
...
$specs = @(
  ...
  @{ n = "paste.rs";  a = @("--data-binary", ("@" + $file), "https://paste.rs") },
  @{ n = "uguu";      a = @("-F", ("files[]=@" + $file), "https://uguu.se/upload?output=text") },
  @{ n = "envs.sh";   a = @("-F", ("file=@" + $file), "https://envs.sh") },
  @{ n = "0x0.st";    a = @("-A", "Mozilla/5.0", "-F", ("file=@" + $file), "https://0x0.st") },
  @{ n = "tmpfiles";  a = @("-F", ("file=@" + $file), "https://tmpfiles.org/api/v1/upload") }
)
...
foreach ($sp in $specs) {
  ...
  try {
    $raw = & $curl -s -m 300 @argl 2>$null | Out-String
    $link = Link-Of $raw
    if (-not $link) {
      $t = $raw.Trim()
...
if ($good.Count -gt 0) {
  ...
  Write-Host ("sha16 : " + $hash)
  try { Set-Content -Path $out -Value $good -Encoding UTF8 } catch { }
} else {
