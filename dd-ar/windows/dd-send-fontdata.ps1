...
Write-Host ""
Write-Host "Double Dealers  -  packing the three other fonts" -ForegroundColor Cyan
Write-Host ""
...
foreach ($s in $specs) {
  ...
  $who = ""
  if ($lo -gt 0) { $who = NameAt $bytes ($at + $lo) ($at + $no) } else { $who = NameAt $bytes $at ($at + 4) }
  Write-Host ("  " + $nm.PadRight(7) + " at " + $at.ToString().PadLeft(9) + "  " + $sz.ToString().PadLeft(8) + "  " + $who)
  $t2 = $who
